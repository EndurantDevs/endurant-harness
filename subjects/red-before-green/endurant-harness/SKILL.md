...
For clear reversible work, inspect instructions/status, path/test, files/checks. Behavior bugs require a failing regression before production edit. Fix; run focused behavior, CI, and diff. Skip probe, hypothesis, analogy, checkpoint, JSON plan, subagent, and broad suites. Escalate for uncertainty, contradictions, coupling, performance/efficiency, migrations, security, deployment, or material risk.
