...
def run_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for stage in stages:
        ...
        remaining = deadline_at - time.monotonic() if enforce_deadline else None
        if remaining is not None and remaining <= 0:
            ...
            for command in commands:
                ...
                if remaining is not None:
                    adjusted["timeout"] = min(adjusted["timeout"], max(0.05, remaining))
                execution_commands.append(adjusted)
            ...
            for command in commands:
                adjusted = dict(command)
                if enforce_deadline:
                    command_remaining = deadline_at - time.monotonic()
                    if command_remaining <= 0:
                        ...
                        deadline_exhausted = True
                        continue
                    adjusted["timeout"] = min(
                        adjusted["timeout"], max(0.05, command_remaining)
                    )
                command_results.append(
                    _execute_command(
                        stage["name"], adjusted, root, log_dir, args.max_tail_lines
                    )
                )
    ...
    if deadline_exhausted:
        prior_failed = True
        result["proof_error"] = "plan proof deadline exhausted"
