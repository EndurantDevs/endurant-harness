...
def _run_capture(argv: Sequence[str], cwd: Path, timeout: int = 6) -> tuple[int, str]:
    try:
        ...
        if len(output) > MAX_TIMEOUT_OUTPUT_CHARS:
            output = "<earlier timeout output omitted>\n" + output[-MAX_TIMEOUT_OUTPUT_CHARS:]
...
def _scan_repository(
    root: Path, max_depth: int, max_items: int, *, recursive: bool = True
) -> dict[str, Any]:
    project: list[str] = []
    ci: list[str] = []
    top: list[str] = []
...
def _candidate_paths(root: Path, task: str, max_items: int) -> tuple[list[str], list[str]]:
    ...
    argv.extend([pattern, "."])
    code, output = _run_capture(argv, root, timeout=2)
    if code not in {0, 1}:
