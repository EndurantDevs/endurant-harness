...
def _execute_command(
    ...
    if status == "passed":
        assertion_tail = _tail_lines(log_path, 1_000_000) if log_path.exists() else []
        bounded_output = "\n".join(assertion_tail)
        must_match = command.get("must_match")
        must_not_match = command.get("must_not_match")
        if must_match and re.search(must_match, bounded_output, flags=re.MULTILINE) is None:
            status = "failed"
            error = f"output did not match required regex: {must_match}"
        elif must_not_match and re.search(must_not_match, bounded_output, flags=re.MULTILINE):
            status = "failed"
            error = f"output matched forbidden regex: {must_not_match}"
