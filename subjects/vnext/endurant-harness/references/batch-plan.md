...
The standard-library runner stores complete output in a new private temporary log directory and emits a bounded evidence summary. `--log-dir` selects the parent for that private directory; use a location outside the repository or one already ignored by Git when proof is fingerprint-bound.
...
- `proof_deadline_seconds` caps proof commands at the remaining plan budget; `always` cleanup still runs after exhaustion.
...
- Use `must_match` or `must_not_match` only for summary-at-end assertions; matching is limited to the final 64 KiB of output.
- For high-risk proof, set `expected_diff_fingerprint` from `endurant.py fingerprint --repo .`; the runner rejects stale or proof-mutated diffs. Keep logs outside the repository.
