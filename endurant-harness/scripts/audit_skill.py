...
REQUIRED_CAPABILITIES = {
    ...
    'runner-safety',
    'direct-lane-classification',
    'task-selected-verification',
    'runner-output-assertions',
    'total-proof-deadline',
    'high-risk-diff-fingerprint',
    'aggregate-workspace-probe-diet',
    'conditional-red-first',
    'symbol-first-probe-relevance',
    'repository-fast-preflight-contract',
    'repository-benchmark-contract',
    'version-session-provenance',
...
def validate_provenance(root: Path, result: dict[str, Any]) -> None:
    skill_path = root / "SKILL.md"
    try:
        text = skill_path.read_text(encoding="utf-8")
    except OSError as exc:
        add(result, "errors", "provenance.read", f"cannot read SKILL.md: {exc}")
        return
    matches = list(PROVENANCE_MARKER_RE.finditer(text))
    if len(matches) != 1:
        add(
            result,
            "errors",
        ...
        return
    release, package_sha256 = matches[0].groups()
    ...
    if (
        ...
        or payload.get("state") != "current"
        or payload.get("package_integrity") is not True
        ...
        add(
            ...
            f"provenance marker does not match the canonical package: {checked.stderr or checked.stdout}",
        )
        return
    result["metrics"]["provenance_release"] = release
    result["metrics"]["package_sha256"] = package_sha256
...
def audit(skill_root: Path, strict: bool, smoke: bool) -> dict[str, Any]:
    ...
    if strict:
        validate_provenance(root, result)
